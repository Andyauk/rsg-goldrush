...
AddEventHandler('rsg-goldrush:server:reward', function()
	...
	if foundgold < Config.GoldChance then
		...
		if chance <= 50 then
			...
			TriggerClientEvent('ox_lib:notify', src, {title = 'Info', description = 'not much in this pan', type = 'inform', duration = 5000 })
		elseif chance >= 50 and chance <= 80 then -- medium reward
			...
			TriggerClientEvent('ox_lib:notify', src, {title = 'Info', description = 'looks like good gold', type = 'inform', duration = 5000 })
		elseif chance > 80 then -- large reward
			...
			TriggerClientEvent('ox_lib:notify', src, {title = 'Info', description = 'gold fever jackpot..', type = 'inform', duration = 5000 })
		end
	else
		TriggerClientEvent('ox_lib:notify', src, {title = 'Info', description = 'no gold this time..', type = 'inform', duration = 5000 })
	end
end)
